#pragma once
// ============================================================
// binary_core.h  --  Core two-stage binary-endpoint design computations
//
...
// MAIN SEARCH
// ============================================================
inline TwoStageDesigns binary_BUDS_engine(
    ...
    double robust_expand_max = 2.0)
{
            ...
            for (int r1 = 0; r1 <= n1; ++r1) {
                ...
                for (int rt = r1 + n2; rt >= r1; --rt) {
                    ...
                    for (int rb = r1 + 1; rb <= rb_hi; ++rb)
                        b2 += lut_n1_p1.pmf[rb] * lut_n2_p1.pb(rt - rb); //we have rb responses in stage 1; need rt-rb responses to reject H0 in stage 2
                    // we want to stop once beta1 + beta2 ≤ beta
    ...
    bool has_BUDS = !robust_cands.empty();
    if (!has_classic && !has_BUDS) std::fprintf(stderr, "No design feasible.\n");
    else if (has_classic && !has_BUDS) std::fprintf(stderr, "No BUDS design feasible.\n");
